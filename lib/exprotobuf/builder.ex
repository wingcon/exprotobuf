defmodule Protobuf.Builder do
  ...
  def generate(msgs, config) do
    ...
    quotes =
      for {{item_type, item_name}, fields} <- msgs,
        ...
        if only != [] do
          ...
          if last_mod in only or is_child? do
            case item_type do
              ...
              :msg ->
                def_message(ns, fields, inject: inject, doc: doc, syntax: :proto2)

              :proto3_msg when is_child? ->
                item_name = fix_ns(item_name, ns)
              ...
              :proto3_msg ->
                def_message(ns, fields, inject: inject, doc: doc, syntax: :proto3)

              :enum when is_child? ->
                item_name = fix_ns(item_name, ns)
                def_enum(item_name, fields, inject: false, doc: doc)

              :enum ->
                def_enum(ns, fields, inject: inject, doc: doc)
          ...
          case item_type do
            :msg ->
              def_message(item_name, fields, inject: false, doc: doc, syntax: :proto2)

            :proto3_msg ->
              def_message(item_name, fields, inject: false, doc: doc, syntax: :proto3)
    ...
    unified_msgs = Enum.map(msgs, &unify_msg_types/1)
  ...
  defp fix_ns(name, ns) do
    ...
    name_parts = name_parts -- ns_parts
